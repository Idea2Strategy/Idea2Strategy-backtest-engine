"""Startup verification that the live schema matches the code.

The runtime never applies DDL, so the only safe alternative is to check and refuse.
`verify_schema` reports *every* problem it finds in one exception rather than dying on
the first, because a partially-applied bundle usually breaks several tables at once.
"""
...
def _render(type_: TypeEngine[object]) -> str:
    # `TypeEngine.compile` carries no annotations in SQLAlchemy 2.0.
...
def describe_schema_drift(connection: Connection, metadata: MetaData = METADATA) -> list[str]:
    """Return a list of human-readable drift descriptions; empty means healthy."""
    ...
    for table in metadata.sorted_tables:
        ...
        for column in table.columns:
            ...
            if isinstance(column.type, ENUM):
                ...
                # `str(type)` loses `WITH TIME ZONE`; compile through the dialect instead.
                actual_type = _render(found["type"])
                if expected_type != actual_type:
                    problems.append(f"{qualified}.{column.name}: type is {actual_type}, expected {expected_type}")
...
def verify_schema(connection: Connection, metadata: MetaData = METADATA) -> None:
    ...
    if problems:
        raise SchemaDriftError(
            "the live database does not match the canonical backtest schema:\n  - " + "\n  - ".join(problems)
        )
...
def _enum_problems(
    ...
    if actual != expected:
        yield (
            f"{qualified}.{column_name}: enum {key[0]}.{key[1]} has labels {list(actual)}, expected {list(expected)}"
        )
...
def _unique_problems(inspector: Inspector, table: Table, qualified: str) -> Iterable[str]:
    ...
    for missing in sorted(expected - actual, key=lambda columns: sorted(columns)):

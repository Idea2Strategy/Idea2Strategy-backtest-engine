...
`feature_materialization_version` and `execution_policy_version` are arguments of the
SHA-256 in `runs.configuration_hash` and cannot be recovered from it. It is written in
...
class DurableBacktestResultQueryStore:
        ...
        try:
            return RunProjection(
                ...
                inputs=RunInputs(
                    ...
                    input_bundle_fingerprint=row.configuration_hash,
    ...
    def _check_result_agrees(
        ...
        if result.run_snapshot.input_bundle_fingerprint != row.configuration_hash.removeprefix("sha256:"):
            raise QueryIntegrityError(
                f"run {row.id} pinned {row.configuration_hash} but the stored evidence was "
